...
class AsyncDockerContainerManager:
    ...
    async def _build_image(self, build_context_dir: str, image_name: str, node_idx: int, nocache: bool = False):
        ...
        try:
            ...
            async def read_stream(stream, prefix="", lines_list=None):
                while True:
            ...
            async def build_with_timeout():
                await asyncio.gather(
                    read_stream(process.stdout, lines_list=stdout_lines),
                    read_stream(process.stderr, "[ERROR] ", lines_list=stderr_lines)
                )
                await process.wait()
            ...
            if process.returncode != 0:
                ...
                # Log the actual build output at ERROR level so it's visible
                if stderr_lines:
                    self.logger.error("Build stderr output:")
                    for line in stderr_lines[-50:]:  # Last 50 lines
                        self.logger.error(f"  {line}")
                if stdout_lines:
                    self.logger.error("Build stdout output (last 20 lines):")
