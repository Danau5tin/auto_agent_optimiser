...
"""
    ...
    async def run_bash(self, command: str) -> str:
        ...
        try:
            result = await self.env.exec(command)
            output_parts = []
            if result.stdout:
                output_parts.append(self._clean_bash_output(result.stdout))
            if result.stderr:
                output_parts.append(f"stderr: {self._clean_bash_output(result.stderr)}")
            if result.return_code != 0:
