"""
Test scenario: Agent can add a tool which is clearly missing.
In this case, the target system is a coding agent which needs to add a bash tool to complete
"""
...
class TestCodingAgent(BaseScenario):
    """
    Scenario where a coding agent attempts real TerminalBench 2.0 evals but does not have a bash tool which the optimiser agent is expected to add.
    """
    ...
    def asset_dir(self) -> str:
        return "coding_agent_with_no_bash_tool"

    @property
    def default_evals(self) -> list[str] | None:
        """Default evals to run when 'all' is requested. Override in subclasses."""
        return ["fix-git"]

    @property
    def tbench_config(self) -> TBenchConfig:
        """TBench configuration. Override to customize."""
        return TBenchConfig(
    ...
    async def setup(self) -> None:
        ...
        self.temp_dir = Path(tempfile.mkdtemp(prefix="coding_agent_no_bash_tool_"))
    ...
    async def verify_success(self, file_manager: BaseFileManager) -> None:
        """Verify that the agent has a bash tool after optimisation."""
        # Check tool_call_entities.py for a bash-related tool call class
        tool_entities_content, _ = await file_manager.read_file("tool_call_entities.py")
        has_bash_tool_class = any(
            keyword in tool_entities_content.lower()
            for keyword in ["bashtoolcall", "bash_tool", "bashcall", "class bash"]
        )

        # Check agent.py for bash in the tool call parser mapping
        agent_content, _ = await file_manager.read_file("agent.py")
        has_bash_in_mapping = any(
            keyword in agent_content.lower()
            for keyword in ['"bash"', "'bash'", "bash:", "bashtoolcall"]
        )

        if not has_bash_tool_class:
            raise AssertionError(
                "Verification failed: No bash tool call class found in tool_call_entities.py"
            )

        if not has_bash_in_mapping:
            raise AssertionError(
                "Verification failed: No bash action type found in agent.py tool_call_parser mapping"
            )
